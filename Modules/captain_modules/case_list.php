...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_action_only'])) {
  ...
  $stmt->bind_param("si", $action_taken, $case_number);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_case'])) {
    ...
    // basic validations
    if (!ctype_digit($case_number)) {
    swal_back('Case number must be numeric.');
}

// required fields
if (!$case_number || !$nature_offense || !$date_filed || !$time_filed || !$date_hearing || !$action_taken
...
<div class="d-flex justify-content-between align-items-center mb-3">
  ...
  <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#addCaseModal" id="addCaseTrigger">➕ Add Case</button>
</div>
