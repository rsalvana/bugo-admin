...
if (isset($_GET['export_errors']) && $_GET['export_errors'] == '1') {
    if (!empty($_SESSION['lupon_import_errors'])) {
        ...
        foreach ($_SESSION['lupon_import_errors'] as $row) {
            fputcsv($output, $row);
        }
...
register_shutdown_function(function () {
    ...
    if ($error && in_array($error['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR])) {
        ...
        echo "500 Server Error: " . $error['message'];
        exit();
    }
});
...
if (isset($_POST['action']) && $_POST['action'] === 'import_cases') {
    try {
        ...
        for ($r = 2; $r <= $highestRow; $r++) {
            ...
            if ($cl === '' || $rl === '') {
                $errorLog[] = [$r, 'Incomplete Name (Last Name required)', $case_number, $dateFiled, $compRaw, $respRaw];
                $skippedCount++; continue;
            }
        ...
        <table class="table table-striped table-hover table-bordered align-middle mb-0">
          <thead>
            <tr>
              ...
              <th>Date Filed</th><th>Time</th><th>Hearing</th><th>Status</th><th>Action</th>
            </tr>
          </thead>
            ...
            <tr>
                ...
                <?php else: ?>
                    <form method="POST" action="" onsubmit="return confirmUpdate();">
                        ...
                        <select name="action_taken" class="form-select form-select-sm mb-1">
                        ...
                        <button type="submit" name="update_action_only" class="btn btn-primary btn-sm w-100">Update</button>
                    </form>
                <?php endif; ?>
                </td>
                <td>
