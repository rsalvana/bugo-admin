...
function printReport() {
  ...
  if (type === "cases") {
    const win = window.open(`load_report/load_cases_report.php?print=1`, '_blank');
    win.onload = () => setTimeout(() => win.print(), 500);
  } else if (type === "schedules") {
    const cert = document.getElementById("reportCertificate")?.value || "";
    const year = document.getElementById("reportYear")?.value || "";
    const win = window.open(`load_report/load_schedules_report.php?print=1&certificate=${encodeURIComponent(cert)}&year=${encodeURIComponent(year)}`, '_blank');
    win.onload = () => setTimeout(() => win.print(), 500);
  } else {
    window.print();
  }
