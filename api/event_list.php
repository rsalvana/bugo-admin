...
$todayYmd = $nowPH->format('Y-m-d');          // PH today for <input min>
...
<div class="container my-5 table-responsive">
  ...
  <form method="GET" action="index_Admin.php" class="row g-2 mb-3 filter-form">
          ...
          <div class="row g-3">
            <div class="col-lg-7">
              <div class="mb-3">
                ...
                <input type="text" name="event_name" id="event_name" class="form-control" placeholder="e.g., Barangay Outreach Fair" required>
                <div class="form-text field-note">Type the full name of the event.</div>
              </div>
          ...
          <div class="row g-3">
            <div class="col-lg-7">
              <div class="mb-3">
                ...
                <input type="text" class="form-control" name="event_name" id="edit_event_name" placeholder="e.g., Community Cleanup Day" required>
              </div>
...
<script>
/* PH “now” that keeps ticking on client, anchored to server time */
const BASE_PH_MS     = <?= $nowPHms ?>;  // server Asia/Manila epoch ms
const BASE_CLIENT_MS = Date.now();
function nowPHDate(){ return new Date(BASE_PH_MS + (Date.now() - BASE_CLIENT_MS)); }
function ymdPH(){ const d = nowPHDate(); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`; }
function hhmmPH(){ const d = nowPHDate(); return `${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`; }
function toAmPm(hhmm){ const [h,m]=hhmm.split(':').map(Number); const ampm=h>=12?'PM':'AM'; const h12=(h%12)||12; return `${h12}:${String(m).padStart(2,'0')} ${ampm}`; }
...
function revalidateAdd(){
  ...
  setInvalid(document.getElementById('start_time'), false);
  setInvalid(document.getElementById('end_time'), false);

  let ok = true;
  if (d && s && d === nowD && s < nowT){
    ...
    setInvalid(document.getElementById('start_time'), true);
    ok = false;
  }
  if (s && n && !isStartBeforeEnd(s,n)){
    timeErr.classList.remove('d-none');
    setInvalid(document.getElementById('end_time'), true);
    ok = false;
  }
  document.getElementById('addDuration').textContent = humanDuration(s, n);
  return ok;
}
...
function validateAddDateTime(){
  ...
  if (!dateOk) { Swal.fire('Invalid','Please select today or a future date (Asia/Manila).','warning'); return false; }
  if (!revalidateAdd()){ Swal.fire('Invalid','Please fix the time errors.','warning'); return false; }
  return true;
}
...
function revalidateEdit(){
  ...
  nowErr.classList.add('d-none'); timeErr.classList.add('d-none');
  setInvalid(document.getElementById('edit_start_time'), false);
  setInvalid(document.getElementById('edit_end_time'), false);

  let ok = true;
  if (d && s && d === nowD && s < nowT){
    nowErr.textContent = `Start time cannot be earlier than the current time in Asia/Manila (${toAmPm(nowT)}).`;
    nowErr.classList.remove('d-none');
    setInvalid(document.getElementById('edit_start_time'), true);
    ok = false;
  }
  if (s && n && !isStartBeforeEnd(s,n)){
    timeErr.classList.remove('d-none');
    setInvalid(document.getElementById('edit_end_time'), true);
    ok = false;
  }
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  document.querySelectorAll('.view-edit-btn').forEach(btn=>{
    btn.addEventListener('click', async function(){
      ...
      try{
        ...
        document.getElementById('edit_event_name').value = ev.event_name || ''; // <-- MODIFIED
        ...
        revalidateEdit(); // run against current PH time immediately
      }catch(err){
  ...
  if (editForm){
    editForm.addEventListener('submit', async function(e){
      ...
      if (!dateOk || !revalidateEdit()){
        e.preventDefault();
        Swal.fire('Invalid', 'Please fix the time/date errors.', 'warning');
        return;
      }
      e.preventDefault();
...
// Keep for other modules that might call it
function ymdToday(){ return ymdPH(); }
</script>
</body>
</html>